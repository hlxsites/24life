...
async function main() {
    const response = await fetch("https://main--24life--hlxsites.hlx.page/drafts/import-status.json?limit=9999");
    const json = await response.json();
    let urls = json.data.map(row => row["Reference"])
